$(function(){
  ...
  //获取返回的url
    ...
    var fundCode = $("#fundCode").html();
    $.ajax({
      url:'account/register',
      type:'post',
      dataType:'text',
      data:{tel:telObj.val(),loginPwd:loginPwdObj.val(),verifyCode:verifyCodeObj.val(),identifyCode:identifyCodeObj.val(),returnFlag:returnFlag,productId:productId,fundCode:fundCode},
      error: function(){
        errorMsg = " 请求失败，请稍后再试";
        openLayer(errorMsg);
      }, 
...
function openLayer(errorMsg){
  ...
  $("#errorMsgMask").remove();
  $("#failMask").css("top","20%");
  $("#layerBody").append("<p id='failTitle' class='failTitle'>注册失败</p>");
