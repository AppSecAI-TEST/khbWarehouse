...
//未登录充值
function toNoLoginRecharge(){
  var u = navigator.userAgent;
  var isApp = /lanmao/i.test(u);
  if(isApp) {
    location.href ="account/toLogin?returnUrl=asset/toRecharge?ret=seckillPer";
    return;
  }
  location.href ="asset/toRecharge?ret=seckillPer";
}
function toBuyNoLogin () {// 点击立即秒杀
//  var productId = $("#productId").val();
  //APP端不弹出蒙层，直接被原生登录拦截
  var u = navigator.userAgent;
  var isApp = /lanmao/i.test(u);
  if(isApp) {
    location.href ="account/toLogin?returnUrl=seckillActivity/toSeckillPro";
    return;
  }
  $("#alertLayer-5 .pr a").eq(2).attr('href',
      "account/toLogin?returnUrl=seckillActivity/toSeckillPro");
  $("#alertLayer-5 .pr a").eq(1).attr('href',
      "account/toRegister?returnFlag=toSeckill");
  $("#mask").show();
  $("#alertLayer-5").show();
}
function toBuy () {
...
//	    + "&ret=fixed" + "&expectIncome=" + expectIncome;
      ...
      + "&ret=seckill" + "&expectIncome=" + expectIncome;
	  return;
  }
  if (rechargeAmount == 0) {// 去确认购买页面
    ...
    location.href = "asset/toRecharge?amount=" + rechargeAmount
        + "&ret=seckill&productId=" + productId + "&expectIncome=" + expectIncome
        + "&promoNo=" + promoNo;
  }
}
//购买金额失去焦点
function getExpectIncome () {
  ...
  var surplusAmount = parseFloat($("#surplusAmount").val());
  var errorMsg = null;
  var flag = false;
  var buyAmount = 0;
...
*/    //由于懒猫账号余额可能有小数点后三位的值，即厘，故用去尾法
    var rechargeAmountFloat = parseFloat(buyAmount)- parseFloat(accountAmount).toFixed(2);
