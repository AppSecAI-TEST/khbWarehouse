...
import com.yeepay.g3.facade.zt.enums.PolicyOrderDetailOrderStatusEnum;
...
public class MyPolicyInvestAssetAction extends BaseAction{
	...
	private ZtPurchaseFacade ztPurchaseFacade = RemoteServiceFactory.getService(ZtPurchaseFacade.class);
...
//		String memberNo = "871234567362";
		...
		try{
			...
			List<ZtPolicyFundShareDTO> fundSharelist = ztPolicyFundShareFacade.queryFundAndPolicyProductList(Long.valueOf(poicyOrderId),memberNo);
...
//			ztPolicyOrderDetailPageDTO= ztPolicyOrderDetailFacade.queryUserPolOrdDetailByPage("871234567362", policyOrderId, orderType, orderStatus, PAGE_SIZE, pageIndex);
	...
	public String toPolicyFundOrderDetail(@Param("policyOrderDetailId") Long policyOrderDetailId){
		...
		List<ZtPurFeeDetailDTO> resultList = null;
		List<ZtUserPolicyFundPropertionDTO> fundList = new ArrayList<ZtUserPolicyFundPropertionDTO>();
		...
		try{
			...
			for(ZtPolicyFundOrderDetailDTO dto : list){
				...
				ZtUserPolicyFundPropertionDTO zpfpDto = new ZtUserPolicyFundPropertionDTO();
				zpfpDto.setFundCode(dto.getFundCode());
				zpfpDto.setAdditionalAmount(dto.getBalance());
				fundList.add(zpfpDto);
			}
			if(ztPolicyOrderDetailDTO != null){
				//策略订单没有完全确认，预估手续费
				if(!PolicyOrderDetailOrderStatusEnum.CONFIRMED.equals(ztPolicyOrderDetailDTO.getOrderStatus())){
					resultList = ztPurchaseFacade.queryPurchaseFeeDetail(fundList);
				}
			}
			//不为null，说明策略订单未完全确认
			if(resultList != null){
				for(ZtPurFeeDetailDTO ztfeeDto : resultList){
					fee = fee.add(ztfeeDto.getRealFareSx());
				}
			}else{
				//为null，可能策略订单完全确认或查询预估手续费失败，这是手续费都以策略订单手续费为准
				fee = ztPolicyOrderDetailDTO.getFee();
			}
...
//		String memberNo = "123456";
		...
		try{
			//TODO
			totalIncome = ztPolicyOrderFacade.queryTotalIncomeByMemberNo(memberNo);
			if(totalIncome == null){
