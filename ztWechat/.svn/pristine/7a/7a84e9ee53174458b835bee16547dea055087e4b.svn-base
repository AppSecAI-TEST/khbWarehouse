   ...
   <p class="evalTitle">
    ...
    <e:iterator list="@dtoList" var="item">
	    <div class="evalMain evalMain-1 bg-white">
	        ...
	        <h3><a href="javascript:void(0)"><e:property value="@item.policyName"/>·策略<span class="fr wish-title">愿望:${ztSceneDto.sceneName }</span></a></h3>
	        </a>
	        <a href="zt/policy/toPolicyInfo?sceneId=${sceneId}&calculateRecordId=<e:property value="@item.id"/>">
	        ...
	        <div class="evalDetail">
	            <p class="wish-text">您需投入<span class="wish-num orange"><e:property value="@item.perInvestAmount"/></span>元/月，连续投资<span class="wish-num-1 orange"><e:property value="@item.totalInvestTerm"/></span>年</p>
	            <div class="invest wish-invest">
	                <p class="total">预期未来总市值(元)</p>
	                <p><e:property value="@item.futureAmountMinStr"/> ~ <e:property value="@item.futureAmountMaxStr"/></p>
	            </div>
	            <div class="rate pr">
	                <ul>
	                    <li class="fl">
...
<div id="alertLayer" class="bg-white" style="display: none; width: 90%; min-height: 20%;">
    <div class="withdrawIntro detailIntro">
        ...
        <div class="btnArea btnArea-new tc">
            <a href="zt/risk/questionsPage?sceneId=${sceneId}" class="singleBtn singleBtn-new fl">重新答题</a>
            <a href="javascript:void(0)"  class="btnClosed singleBtn  fr">先看看投资计划</a>
        </div>
    </div>
</div>
</body>
