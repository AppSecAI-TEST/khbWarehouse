	...
	<div class="Container">
		<div class="Content fontText">
			...
			<div class="information">
				<form id="queryEventForm" method="get"
					action="${ctxPath}event/queryEventList">
					<div class="search">
						...
						<div class="search_con">
							<ul class="fix">
								<li><label class="text_tit">事件编码：</label> <input
									type="text" name="eventCode" class="input_text"
									value="<c:out value="${eventCode}"/>" /></li>
								<li><label class="text_tit">事件名称：</label> <input
									type="text" name="eventName" class="input_text"
									value="<c:out value="${eventName}"/>" /></li>
							</ul>
							<div class="btn">
			...
			<div class="result">
				<h2 class="fw">查询结果</h2>
				<q:table queryService="activityQueryService" queryKey="queryEventList"
					template="querytableForUnion" formId="queryEventForm"
					class="table_list" pageSize="15">
					<q:nodata>无符合条件的记录</q:nodata>
					<q:column title="序号" value="${id}" />
					<q:column title="事件编码" value="${event_code}" />
					<q:column title="事件名称" value="${event_name}" />
					<q:column title="创建时间" value="${_messageFormater.formatDate(create_time)}" />
					<q:column title="创建人" value="${creator}" />
					<q:column title="操作人" escapeHtml="false">
