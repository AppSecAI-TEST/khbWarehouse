...
$(document).ready(function() {
	...
	$("#dataExportBtn").click(function() {
		if(confirm('是否确定下载文件?')){
		  window.location.href=GV.ctxPath+"ztPolicy/exportTransactionOrderList?" + $("#queryTransactionOrderListForm").serialize();
		}
	});
	
	$("#sumBtn").click(function() {
		$("#sum_span").html("统计中……");
	    $.post(GV.ctxPath+"ztPolicy/queryTransactionOrderTotal", $("#queryTransactionOrderListForm").serialize(), 
	        function(result) {
	      $("#sum_span").html("笔数：" + result.total_count + " 金额：" + result.total_amount);
	    }, "json");
	});
})
	...
	<div class="Container">
		<div class="Content fontText">
			...
			<div class="information">
				<form id="queryTransactionOrderListForm" method="get"
					action="${ctxPath}ztPolicy/queryTransactionOrder">
					<div class="search">
						...
						<div class="search_con">
							...
							<div class="btn">
								...
								<input id="sumBtn" type="button" class="btn_sure fw" value="统计" />
								&nbsp;&nbsp;&nbsp;&nbsp;<span id="sum_span" style="color:red;"></span>
								<div style="font-size:15px;float: right"><a id ="dataExportBtn" href="javascript:void(0);">导出数据</a></div>
							</div>
			...
			<div class="result">
				<h2 class="fw">查询结果</h2>
				<q:table queryService="ztPolicyQueryService" queryKey="queryTransactionOrderList"
					template="querytableForUnion" formId="queryTransactionOrderListForm"
					class="table_list" pageSize="15">
					<q:nodata>无符合条件的记录</q:nodata>
					<q:column title="策略名称" value="${policy_name}" />
					<q:column title="用户编号" value="${member_no}" 	/>
					<q:column title="策略交易汇总ID" value="${policy_order_id}" />
					<q:column title="交易类型" >
