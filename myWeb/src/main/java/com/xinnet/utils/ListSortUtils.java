...
public class ListSortUtils {  
	...
	private static final Logger logger = LoggerFactory.getLogger(ListSortUtils.class);
    /** 
     * 对list的元素按照多个属性名称排序, 
    ...
    public static <E> void sort(List<E> list, final String[] sortnameArr, final boolean[] typeArr) {  
        if (sortnameArr.length != typeArr.length) {  
            throw new NumberFormatException("属性数组元素个数和升降序数组元素个数不相等");  
        }  
        Collections.sort(list, new Comparator<E>() {
        	@Override
            public int compare(E a, E b) {  
     ...
     * @throws Exception 
     */  
    private static <E> int compareObject(final String sortname, final boolean isAsc, E a, E b) throws IllegalAccessException {  
        int ret;  
